defmodule Grid.Reflection do
  def horizontal_at?(grid, row_index_a, row_index_b) do
    with {row_a, _} when row_a != nil <- Enum.at(grid, row_index_a),
  ...
  def vertical_at?(grid, column_index_a, column_index_b) do
    Enum.reduce_while(grid, true, fn {row, _i}, _acc ->
      codepoints = String.codepoints(row)

      with column_a when column_a != nil <- Enum.at(codepoints, column_index_a),
           column_b when column_b != nil <- Enum.at(codepoints, column_index_b) do
